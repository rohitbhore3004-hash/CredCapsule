...
import com.danilkinkin.findtrack.LocalWindowInsets
import com.danilkinkin.findtrack.R
import com.danilkinkin.findtrack.base.ButtonRow
import com.danilkinkin.findtrack.base.Divider
import com.danilkinkin.findtrack.base.LocalBottomSheetScrollState
...
import com.danilkinkin.findtrack.data.RestedBudgetDistributionMethod
...
import com.danilkinkin.findtrack.util.*
...
fun Wallet(
    ...
    var balanceCache by remember { mutableStateOf(spendsViewModel.budget.value!!) }
    ...
    val remainingBalance = (balanceCache - spent - spentFromDailyBudget)

    val openConfirmResetWalletDialog = remember { mutableStateOf(false) }
    ...
    val isChange = balanceCache != budget
                    ...
                    Column {
                        ...
                        CompositionLocalProvider(
                            ...
                            ButtonRow(
                                ...
                                onClick = {
                                    openConfirmResetWalletDialog.value = true
                                }
                            )
                        }
                    }
                    ...
                    Column {
                        ...
                        Button(
                            onClick = {
                                ...
                                // Use a far-future date so wallet has no expiry
                                val farFuture = LocalDate.now().plusYears(100).toDate()

                                // Auto-distribute leftover balance so RecalcBudget dialog never pops up
                                spendsViewModel.changeRestedBudgetDistributionMethod(
                                    RestedBudgetDistributionMethod.REST
                                )

                                if (spends!!.isNotEmpty() && !forceChange) {
                                    spendsViewModel.changeBudget(balanceCache, farFuture)
                                } else {
                                    spendsViewModel.setBudget(balanceCache, farFuture)
    ...
    if (openConfirmResetWalletDialog.value) {
        ConfirmFinishEarlyDialog(
            ...
            onClose = { openConfirmResetWalletDialog.value = false },
        )
    }
}
